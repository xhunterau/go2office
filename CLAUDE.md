...
- **只写 SELECT 策略的表，应用是写不进去的——包括你以为在写的日志**：`order_logs`（迁移 `20260810140000`）建表时只给了 `authenticated` 一条 SELECT 策略，理由是"写它的是 service role 下的后台任务"。这个前提后来不成立了，于是 `/fulfillment/export-labels` 的每一次导出都在返回「the order log was not written」——而那条路径按规则 24 把日志失败降级成 warning，所以症状**只是一行提示，不是错误**，从 2026-08-10 到 2026-08-23 没人发现。迁移 `20260823120000` 补了 INSERT 策略（`WITH CHECK (user_id = auth.uid())`，不给 UPDATE / DELETE，审计表只能追加）。**给一张表加"某某场景下要写日志"的需求时，先确认那张表当前的角色写得进去**。
...
- **承运商映射表缺键必须抛 `UnmappableOrderError`，严禁 `?? 默认值`**：`mapPackagingType`（MyPost 包装代码）与 `mapChargeCode`（eParcel 计费代码）都是按 `shipping_method` 取值的表。xpros 分别兜底成 `OWN_PACKAGING` 与 `3D55`——前者把澳邮自有箱按自备包装下单并据此计价，后者把快递件按普邮 code 计费，**承运商照样受理，没有任何东西会暴露它**。两张表现在都是满的，键类型已收窄到各自的通道清单（`MYPOST_METHODS` / `EPARCEL_METHODS`），所以漏配是编译错误；运行时的抛错管的是通道之外的方法，不可改回兜底。
...
- **Aramex 下单不是幂等操作**：`submit-aramex-batch` 的 `retry.maxAttempts` 必须是 1，循环必须**串行**（不是 `Promise.all`）。一次部分失败后重跑，已成功的订单因已变成 `labelled` 会掉出队列，但**在 Aramex 已接受、写库却失败的那一张会被重复下单**。所以 `submitConsignment()` 在写库失败时抛出的错误里必须带上 consignment id。运单号要写回 `orders.tracking_number`（xpros 漏了这步，事后无法把订单和运单对上）——`MS…` 与纯数字都经 `normalize_tracking_number` 安全，唯一的数字分支要求同时以 `99` 开头且长度 > 23（规则 20）。
- **Aramex 响应的字段是 `conId` 与 `items[].label`，不是 `consignmentId`**：xpros 写的 `response.data.consignmentId` 线上**不存在**，它拿到 id 后直接丢掉所以从不暴露；本项目照抄后，首次真实下单把字符串 `"undefined"` 写进了 `tracking_number`（2026-08-23，订单 205971）。`items[0].label`（如 `MS0020719756`）才是客户能查的号，`conId` 只是内部 id。**读不出来时一律写 null，严禁 `String(undefined)` 这类占位值**，订单进结果的 `untracked` 由人工补录——它已经交运，报成失败会诱发重复下单。
- **异步批次的完成状态靠轮询，不靠 realtime 订阅**：`useRealtimeRun` 在 2026-08-23 实测收不到一个正常 COMPLETED 的 run 的终态，页面 spinner 永不停止且四张卡片全被 `busy` 锁死。判定终态必须写成「**不在进行中集合里就是结束**」而不是「等于 COMPLETED 或落在手写的失败集合里」——后者漏了 `EXPIRED`（还带着 v4 不存在的 `INTERRUPTED`），漏一个状态就是一次永久卡死。另需硬超时与手动脱困入口，三者互不依赖。
...
# 25. Order Allocation（/fulfillment/allocation）
- **范围**：`pending` 订单发货前的两道关——地址与运费，由 `src/lib/queries/allocation.ts` + `src/lib/actions/allocation.ts` + `batch-postage-check` Task 支撑。**不做 backorder 检测**（用户决策），所以只有两个阶段、也就是一个 bit 的状态：`orders.address_verified_at`（迁移 `20260824100000`）。队列口径是 `status = 'pending' AND customers.country = 'AU'`，`address_verified_at` 为空进 Address 队列、非空进 Postage 队列，Approve 后进 `processing`（即 `/fulfillment/export-labels` 的队列）。详见 `docs/order-allocation.md`。
- **邮编必须补零，严禁照抄 xpros 的 `LTRIM(postcode,'0')`**：xpros 的参考表丢了前导零（DARWIN 存成 `800`），go2office 的有 CHECK 强制四位。去零会让整个北领地的地址**永远解析不到、永远堆在人工队列里，不报任何错**。`verify_pending_order_addresses()`、`standardize_customer_address`（规则 21）与 `zone-resolver.ts` 的 `normalizePostcode` 三处口径必须逐字一致——TS 侧**直接 import 复用 zone-resolver 的那一份，不许再镜像**。同理 locality 必须等值比较，`ILIKE` 会让 city 里的 `%` 变通配符。
- **批量地址检查是一条 SQL，不是后台任务**：xpros 逐单循环是因为它每单查一次参考表，而这件事是个 JOIN。`verify_pending_order_addresses()` 一条 `UPDATE … EXISTS` 搞定，队列多大都一次往返（与规则 21 的 `004` 同一取舍）。它是 **SECURITY INVOKER**，靠 `auth.uid()` 满足 `order_logs` 的 `WITH CHECK`；改成 DEFINER 会让审计行失去作者。
- **不自动批准任何订单**（用户决策）：xpros 有一套「报价 ≤ 客户已付就自动 Ready」的阈值规则，**不移植**。批量任务只报价，全部等人工。引擎写的 `is_selected` 只作为 `Engine pick` 徽章显示，**不落到 `orders.shipping_method`**；最便宜的另标 `Cheapest`（两者经常不是同一行）。唯一保留的过滤是 `filterFlatRateGroups`，属去冗余。
- **`postage_paid` 是我们付给承运商的，`postage_and_handling` 是客户付给我们的**。xpros 那两列叫 `postage_received` / `postage_paid`，命名恰好错位，照着改极易写反——写反只会让 gross profit 反向偏移，票面上看不出异常。
- **手工批准的方式列表必须收窄到 `MANUAL_APPROVAL_METHODS`**：`Direct_Freight` / `Click_and_Collect` 不属于任何标签通道，批到 `processing` 后会在 `/fulfillment/export-labels` 上永远不出现——订单就这么没了（规则 24 同一类）。单测锁住了这条。
- **引擎会把订单从队列里拿走**：`runQuoteEngine` 报不出价时自己把 `status` 改成 `issued`，订单当场脱离 `pending`。Task 必须单独统计 `escalatedCount` 并在页面单独提示，否则唯一可见的现象是「队列莫名短了几张」。
- **Address 表单不接受 `state` / `country` / `line3` / `line4`**：`state` 会被 `customers_standardize_address` 在保存瞬间重算（规则 21 的静默改写），`country` 改了就脱离 AU-only 队列，`line3` 有 114,161 行是 `ebay:xxxx` 引用码（规则 24）。
- **Places 选完不等于能解析**：Google 返回的 `locality` 未必在 `public.postcodes` 里，`city` 退到 `sublocality_level_1` / `administrative_area_level_2` 时尤其可疑。保存前要复查并对未解析的组合弹确认框，但**不拦住保存**。Maps key 与 xpros 共用，且**当前没有任何 referrer 限制**（2026-08-24 实测三种 referer 均 200）。
