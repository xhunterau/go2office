...
**1–4 已于 2026-08-22 由用户确认，全部维持现状，无需改动任何代码或 seed。**

| # | 问题 | 结论 |
|---:|---|---|
| 1 | `shipping_settings` 五个默认值照搬 xhunter 的 eParcel 合同（1040mm / 22kg / $9.50 超尺寸费 / 1000mm 阈值 / 9.90% 燃油费）—— go2office 是否同一份合同？ | ✅ **同一份合同**。燃油费率与 xpros 保持一致，seed 值即为最终值 |
| 2 | Aramex 的 `max_order_total_aud = 200` 是 xhunter 的保价上限，go2office 是否相同？ | ✅ **相同**，200 不变 |
